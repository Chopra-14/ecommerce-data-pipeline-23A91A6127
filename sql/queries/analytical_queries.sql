...
SELECT
    ...
    ROUND(
        ...
        SUM(COUNT(DISTINCT f.transaction_id)) OVER (), 2
    ) AS pct_of_transactions,
    ROUND(
        SUM(f.line_total) * 100.0 /
        SUM(SUM(f.line_total)) OVER (), 2
    ) AS pct_of_revenue
FROM warehouse.fact_sales f
...
SELECT
    ...
    ROUND(SUM(f.line_total) / COUNT(DISTINCT c.customer_key), 2)
...
SELECT
    ...
    p.category,
    SUM(f.profit) AS total_profit,
    ROUND(SUM(f.profit) / SUM(f.line_total) * 100, 2) AS profit_margin,
